                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('business.services.update', $service) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="add_ons" name="add_ons" class="block mt-1 w-full"
                                      rows="2">{{ old('add_ons', implode(', ', $service->add_ons ?? [])) }}</textarea>
                            <small
                                class="text-gray-500">{{ __('Comma-separated values for additional options') }}</small>
                            @error('add_ons')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
